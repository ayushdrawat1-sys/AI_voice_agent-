...
// Cast motion config to `any` to avoid strict typing mismatches
...
function PlayerBadge({ participant }: { participant?: LocalParticipant }) {
  ...
  useEffect(() => {
    ...
    // Use safe any-cast for on/off to avoid livekit-client types mismatch across versions
    (participant as any).on('nameChanged', updateName);
    (participant as any).on('metadataChanged', updateName);

    return () => {
      (participant as any).off('nameChanged', updateName);
      (participant as any).off('metadataChanged', updateName);
    };
  }, [participant]);
  ...
  return (
    <motion.div
      ...
      bg-white/10 p-2 pr-5 backdrop-blur-md border border-white/10 shadow-lg ring-1 ring-white/10"
    >
      <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-[#FF2D95] to-[#18F3FF] text-[#1A0033] shadow-sm">
        <svg
...
export const SessionView = ({
  ...
  return (
    <section
      ...
      {...props}
    >
      {/* Retro Arcade Gradient & Icons */}
      <div aria-hidden="true" className="absolute inset-0 -z-10 select-none overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-[#1A0033] via-[#FF2D95] to-[#18F3FF]" />

        {/* Floating Icons Background */}
        <FloatingIcon className="top-[10%] left-[10%] w-12 h-12 -rotate-12 opacity-12">
          <TvIcon />
        </FloatingIcon>
        ...
        <FloatingIcon className="bottom-[40%] right-[30%] w-12 h-12 -rotate-[20deg] opacity-6">
          <TvIcon />
        </FloatingIcon>
      </div>
      ...
      <MotionBottom
        {...(BOTTOM_VIEW_MOTION_PROPS as any)}
